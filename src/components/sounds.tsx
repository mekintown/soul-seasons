...
const Sounds = () => {
  ...
  const [backingTrackVolume, setBackingTrackVolume] = useState<number>(defaultBackingTrackVolume)
  ...
  useEffect(() => {
    ...
    if (page in soundPageMap) {
      nextSoundEffect = soundPageMap[page as keyof typeof soundPageMap]?.soundEffect
      nextBackgroundVolume = soundPageMap[page as keyof typeof soundPageMap]?.backgroundVolume
    }
    ...
    if (
      ...
      backingTrackRef.current?.howler.fade(backingTrackVolume, finalVolume, fadeDuration)
      setTimeout(() => {
